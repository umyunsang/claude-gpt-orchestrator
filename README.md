...
- Node.js 20 or newer
- OpenAI's official codex-plugin-cc marketplace and codex plugin
- A working Codex login or account entitlement for the requested model

The CGO source is free and open source. Claude, Codex, GPT access, subscriptions, API usage, or quota may require paid plans.

## Install

The official Codex marketplace must be configured because CGO keeps it as an external dependency rather than copying OpenAI code:
...
~~~text
/plugin marketplace add /absolute/path/to/claude-gpt-orchestrator
/plugin install cgo@claude-gpt-orchestrator-marketplace
~~~

After this repository is published, replace the local path with its GitHub owner/repository:
...
No CGO command is required for normal work. Ask Claude Code as usual:
...
CGO injects the responsibility contract, Claude creates a self-contained brief, and the fixed MCP tool dispatches the matching GPT specialist role.
