...
Before companion discovery or execution, CGO creates or resolves its dedicated plugin-data root, rejects a symbolic link or a directory owned by another user, and enforces mode `0700`. Files created below it may use the official companion's default modes, but the non-traversable root is the local-account isolation boundary.
